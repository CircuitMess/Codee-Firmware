...
std::map<Input::Button, uint8_t> btnBarMap = {{ Input::Button::A,  0 },
									 { Input::Button::B, 1 },
									 { Input::Button::C, 2 }};
...
Game5::Game5(Sprite& base) : Game(base, Games::Dance, "/Games/5", {
		...
		RES_HEART}),
							 ...
							 scoreBar(std::make_shared<GameObject>(std::make_unique<SpriteRC>(PixelDim{ 5, 111 }), nullptr)){

	bottomWall.setPos({ 0, 138 });
	addObject(std::make_shared<GameObject>(bottomWall));

	scoreBar->setPos({ 114, 14 });
	...
	scoreBarSprite->drawRect(0, 0, scoreBarSprite->width(), scoreBarSprite->height(), lgfx::color888(251, 242, 54));
...
void Game5::createNote(uint8_t track){
	auto note = std::make_shared<GameObject>(std::make_unique<StaticRC>(getFile(notesIcon), PixelDim { 5, 5 }),
											 std::make_unique<RectCC>(glm::vec2 { 5, 5 }));

	note->getRenderComponent()->setLayer(2);
	note->setPos({ barsX[track] + 1, -9 });
...
void Game5::noteHit(uint8_t track){
	...
	if(diff <= noteTolerance){
		...
		uint8_t i = rand() % 3;
		playerRC->setAnim(getFile(danceGIFs[i]));

		playerRC->setLoopMode(GIF::LoopMode::Infinite);
		playerRC->setLoopDoneCallback([&](uint32_t){
...
void Game5::adjustScoreBar() const {
	...
	scoreBarSprite->fillRect(1, scoreBarSprite->height() - 1 - infill, scoreBarSprite->width() - 2, infill, lgfx::color888(251, 242, 54));
}
